...
price = _get_price('KORUUSDT')
print(f'KORUUSDT: ${price:.2f}', flush=True)

for tf in ['15m','1h','4h']:
    df = fetch_klines(symbol='KORUUSDT', timeframe=tf, limit=100)
    if df.empty:
...
# Daily price history
df = fetch_klines(symbol='KORUUSDT', timeframe='1d', limit=7)
if not df.empty:
    print(f'\n日线近7天:', flush=True)
